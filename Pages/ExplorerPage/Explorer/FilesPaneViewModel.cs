...
public partial class FilesPaneViewModel : ViewModelBase {
    ...
    private void NavigateTo(string path) {
        ...
        ExplorerList.AddRange(CurrentDirectoryInfo.EnumerateDirectories(SearchString).Select(f => new DirectoryEntry(f.Attributes, f.Name, f.FullName, @"File folder", f.FullName, null, f.CreationTime, f.LastWriteTime, f)));
        ExplorerList.AddRange(CurrentDirectoryInfo.EnumerateFiles(SearchString).Select(f => new FileEntry(f.Attributes, f.Name, f.FullName, f.Extension, f.FullName, f.Length, f.CreationTime, f.LastWriteTime, f)));
    ...
    [RelayCommand] private static void Edit() {
        
    }
    
    [RelayCommand] private static void Execute() {
